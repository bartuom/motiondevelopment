...
function sparkEmitter(spec, resolved) {
  ...
  return {
    ...
    particles: {
      ...
      opacity: { value: { min: .72, max: .98 }, animation: { enable: true, speed: 3.4, sync: false, startValue: 'max', destroy: 'min' } },
      size: { value: spec.size, animation: { enable: true, speed: 5.8, sync: false, startValue: 'max', destroy: 'min' } },
...
function debrisEmitter(spec, resolved) {
  return {
    ...
    particles: {
      ...
      shape: { type: 'square' },
      opacity: { value: { min: .55, max: .86 }, animation: { enable: true, speed: 2.4, sync: false, startValue: 'max', destroy: 'min' } },
      size: { value: spec.size, animation: { enable: true, speed: 3, sync: false, startValue: 'max', destroy: 'min' } },
      rotate: { value: { min: 0, max: 360 }, direction: 'random', animation: { enable: true, speed: 32, sync: false } },
      move: { enable: true, direction: 'right', angle: { value: spec.spread, offset: resolved.directionDegrees }, random: true, straight: false, speed: resolved.speed, outModes: { default: 'destroy' } },
      life: { count: 1, duration: { value: spec.life, sync: false } }
    }
  };
...
function heavyImpactDefinition() {
  ...
  return {
    ...
    async play({ params, particles: particleAdapter, instance }) {
      ...
      const payload = { position: { ...params.position }, direction: { ...params.direction }, directionDegrees: params.directionDegrees, intensity };
      const sparks = { count: Math.max(1, Math.round(spec.sparks.baseCount * intensity)), speed: scaledRange(spec.sparks.speed, speedScale), directionDegrees: params.directionDegrees };
      const debris = { count: Math.max(1, Math.round(spec.debris.baseCount * Math.max(.7, intensity))), speed: scaledRange(spec.debris.speed, speedScale), directionDegrees: params.directionDegrees };

      instance.resolved = {
        intensity, direction: { ...params.direction }, directionDegrees: params.directionDegrees,
        sparks, debris, timings: { ...spec.timings }, duration: spec.duration,
        targetKickPx: 8.5 * intensity, screenKickPx: 4.5 * Math.min(1.5, intensity)
      };
