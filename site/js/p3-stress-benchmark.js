...
const PATH_LABELS = { emitter: 'emitter', shared: 'shared-direct', scheduled: 'shared-scheduled' };
const PROFILE_LABELS = { uniform: 'uniform', heterogeneous: 'heterogeneous', both: 'both' };
...
function makeCounts(total, buckets, profile) {
  return profile === 'heterogeneous' ? weightedDistribute(total, buckets) : distribute(total, buckets);
}
...
async function sampleFrames(adapter, durationMs = 1400) {
  ...
  return { ...summarizeFrames(samples), peakParticles, peakEmitters, peakGroups };
}
...
function setUiBusy(busy) {
  const ids = ['effect-select', 'play-impact', 'play-overlap', 'play-ab', 'run-cancel-gate', 'stop-all', 'particle-path', 'intensity', 'direction', 'stress-load', 'stress-profile'];
  for (const id of ids) {
