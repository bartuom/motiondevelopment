...
- **Status:** ACTIVE — P3.2.1 extends the integrated Shared Emission Scheduler benchmark with a matched heterogeneous Emission Point profile so per-point runtime variation is measured explicitly instead of assuming uniform particles represent production use.
...
- **Next action:** in Runtime Lab **Build P3.2.1**, leave `800 particles / 24 emission points` and `Both — uniform then heterogeneous`, then run **`Synthetic Stress Compare`**. The harness runs both matched profiles through `emitter`, `shared-direct`, and integrated `shared-scheduled`, reports each profile separately, then prints `HETEROGENEITY DELTA` for shared-scheduled. Treat the scheduler as viable only if both profiles are `workload MATCHED`, `cleanup CLEAN`, queued work returns to zero, and heterogeneous parameter variation does not reintroduce long population frames/spikes.
- **Current Runtime Lab:** `site/heavy-impact-lab.html` — P3.2.1
...
- [x] **P3.2.1 heterogeneous Emission Point benchmark:** Runtime Lab can run `uniform`, `heterogeneous`, or `both` profiles. Heterogeneous keeps total particle count and point layout fixed while applying deterministic per-point intensity/count weights (`0.55–1.45`, normalized back to the same total), color, direction, speed, size and opacity variation. Shape and lifetime remain fixed to avoid changing renderer type. `Both` runs uniform then heterogeneous and reports `HETEROGENEITY DELTA` specifically for shared-scheduled.
- [ ] **Run P3.2.1 integrated stress at 800 particles with `Both`.** Each profile must independently report `workload MATCHED` and `cleanup CLEAN`; queued particles must return to zero. Compare shared-scheduled worst population frame/spikes in both profiles and use `HETEROGENEITY DELTA` to quantify the extra cost of per-point variation.
- [ ] **Run Heavy Impact A/B at intensity `2.0` after the integrated scheduler stress passes.** Compare per-play emitter vs shared-scheduled for avg/low/spikes and visually confirm the scheduler does not make hero sparks/debris feel delayed, smeared, or too sparse during the first 1–3 frames.
- [ ] **Shared Emission Points / Burst Pooling adoption decision:** synchronous shared-direct is rejected as default in its current form. Promote shared-scheduled only if uniform + heterogeneous integrated stress and real Heavy Impact all preserve frame pacing, visual timing, ownership, per-point parameters, direction, positioning, lifecycle and cleanup.
...
25. **Shared Emission Points must support heterogeneous runtime data, not just shared topology.** Color, direction, size, speed, opacity and count/intensity may differ per emission point. Production adoption therefore requires matched heterogeneous benchmarking; total particle count must stay controlled so the cost of parameter variation is not confused with simply rendering more particles.
...
- **P3.2.1 — heterogeneous Emission Point stress:** Added `uniform`, `heterogeneous`, and `both` stress profiles. Heterogeneous applies per-point intensity/count weights normalized back to the same total requested particles plus varied color, direction, speed, size and opacity while keeping shape/lifetime fixed. The default `Both` profile automatically runs uniform and heterogeneous matched compares and emits a `HETEROGENEITY DELTA` for shared-scheduled, so per-point flexibility is measured rather than assumed free.
